...
class SwarmConstructionSimulation:
    def generate_seeds(self, line_spacing, agent_radius, origin):
        ...
        self.agents.extend(
            [
                SimulationObject(self.sim.surface, pos, agent_radius, color=seed_color)
                for pos in seed_pos
            ]
        )
        return seed_pos
    ...
    def generate_connecting_agents(self, line_spacing, agent_radius, origin_agent):
        ...
        self.agents.extend(
            [SimulationObject(self.sim.surface, pos, agent_radius) for pos in conn_pos]
        )
    ...
    def generate_cluster_agents(self, line_spacing, agent_radius, origin_agent):
        ...
        for i in range(side_len):
            ...
            for j in range(side_len):
                ...
                self.agents.append(
                    SimulationObject(self.sim.surface, pos, agent_radius)
                )
